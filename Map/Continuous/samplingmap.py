...
class samplingmap(obstacle):
    ...
    def point_is_out(self, point: list) -> bool:
        ...
        :return:        bool
        """
        return min(point) < 0 or point[0] > self.x_size + 1e-2 or point[1] > self.y_size + 1e-2
    ...
    def map_load_continuous_database(self, databaseFile):
        ...
        ''''检测文件头'''
        assert str(self.x_size) == f.readline().strip('\n')[7:]
        assert str(self.y_size) == f.readline().strip('\n')[7:]
